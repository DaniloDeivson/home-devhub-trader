...
export function RobotTechnicalInfo({
  robotId,
  robotName,
...
`;
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Erro ao gerar informações técnicas"
      );
    } finally {
  ...
  function extractMetricsFromText(text: string): TechnicalMetrics {
    ...
    const maxConsecutiveLossesMatch = text.match(
      /max\s*consecutive\s*losses\s*:?\s*(\d+)/i
    );
    if (maxConsecutiveLossesMatch)
    ...
    // Extract best assets
    const bestAssetsSection = text.match(
      /best\s*assets\s*:?\s*([\s\S]*?)(?=best\s*market|worst\s*market|risk\s*level|$)/i
    );
    if (bestAssetsSection) {
      const assetsList = bestAssetsSection[1].match(/[-•*]\s*([^\n]+)/g);
      if (assetsList) {
        result.bestAssets = assetsList
    ...
    const bestMarketSection = text.match(
      /best\s*market\s*conditions\s*:?\s*([\s\S]*?)(?=worst\s*market|risk\s*level|$)/i
    );
    if (bestMarketSection) {
      const conditionsList = bestMarketSection[1].match(/[-•*]\s*([^\n]+)/g);
      if (conditionsList) {
        result.bestMarketConditions = conditionsList
          .map((item) => item.replace(/[-•*]\s*/, "").trim())
          .filter((item) => item.length > 0);
      } else {
        // Try to extract comma-separated list
        ...
        if (conditionsText) {
          result.bestMarketConditions = conditionsText
            .split(/,\s*/)
            .map((item) => item.trim())
            .filter((item) => item.length > 0);
        }
    ...
    const worstMarketSection = text.match(
      /worst\s*market\s*conditions\s*:?\s*([\s\S]*?)(?=risk\s*level|$)/i
    );
    if (worstMarketSection) {
      const conditionsList = worstMarketSection[1].match(/[-•*]\s*([^\n]+)/g);
      if (conditionsList) {
        result.worstMarketConditions = conditionsList
          .map((item) => item.replace(/[-•*]\s*/, "").trim())
          .filter((item) => item.length > 0);
      } else {
        // Try to extract comma-separated list
        ...
        if (conditionsText) {
          result.worstMarketConditions = conditionsText
            .split(/,\s*/)
            .map((item) => item.trim())
            .filter((item) => item.length > 0);
        }
...
`;
    ...
    a.href = url;
    a.download = `ficha-tecnica-${robotName
      .replace(/\s+/g, "-")
      .toLowerCase()}.txt`;
                ...
                <div className="mb-4">
                  ...
                  <div
                    className={`inline-block px-2 py-1 rounded-full text-sm font-medium ${
                      metrics.riskLevel.toLowerCase().includes("alto")
                        ? "bg-red-500 bg-opacity-20 text-red-400"
                        : metrics.riskLevel.toLowerCase().includes("médio")
                  ...
                  <div className="bg-gray-700 p-3 rounded-lg">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {metrics.bestAssets.map((asset, index) => (
                        <span
                          key={index}
                          className="px-2 py-0.5 bg-blue-900 bg-opacity-50 rounded text-blue-300 text-xs"
                        >
                          {asset}
                        </span>
                      ))}
          ...
          <div className="mt-4 p-3 bg-blue-900 bg-opacity-20 border border-blue-800 rounded-lg">
            <div className="flex items-start">
              ...
              <p className="text-sm text-blue-300">
                <strong>Nota:</strong> Estas métricas são estimativas baseadas
                na análise do código do robô e podem variar dependendo das
                condições de mercado, configurações e outros fatores.
                Recomendamos realizar backtests e testes em conta demo antes de
                utilizar o robô em operações reais.
              </p>
            </div>
          </div>
        </div>
